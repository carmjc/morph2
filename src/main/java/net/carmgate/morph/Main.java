...
public class Main {
	...
	//		if (lastSelectedShip != null && lastSelectedShip == globalModel.getSelectedShip()) {
	...
	private void initGL(int width, int height) {
		...
		GLU.gluOrtho2D(focalPoint.x - width / viewport.getZoomFactor() / 2,
				focalPoint.x + width / viewport.getZoomFactor() / 2,
				focalPoint.y + height / viewport.getZoomFactor() / 2,
				focalPoint.y - height / viewport.getZoomFactor() / 2);
	...
	public void start() {
		...
		while (true) {
			...
			//			Vect3D worldMousePos = new Vect3D(GameMouse.getXInWorld(), GameMouse.getYInWorld(), 0);
			// Get mouse position in pixels on the display.
			// TODO is this necessary ?
			//			Vect3D mousePos = new Vect3D(WorldMouse.getX(), WorldMouse.getY(), 0);
			...
			if (Mouse.next()) {
				...
				if (Mouse.getEventButton() == 0) {
					if (Mouse.getEventButtonState()) {
						...
						// Handle drag if the mouse position delta has reached the threshold.
						// TODO implement this
			...
			// FIXME we might be dragging for something else than moving the scene around.
			...
			//				if (Math.abs(holdWorldMousePos.x - MouseInWorld.getX()) > MIN_MOVE_FOR_DRAG || Math.abs(holdWorldMousePos.y - MouseInWorld.getY()) > MIN_MOVE_FOR_DRAG) {
			...
			//				if (Mouse.getEventButton() == 1 && !Mouse.getEventButtonState() && globalModel.getSelectedShip() != null && !World.combat) {
			//					// Right mouse button has been released and a ship is selected
			//					// Activate or deactivate the morph under mouse pointer.
			//					for (Morph morph : globalModel.getSelectedShip().getSelectedMorphList()) {
			...
			//					if (globalModel.getSelectedShip().getSelectedMorphList().isEmpty()) {
			...
			//						}
			//
			//						iaList.add(new FixedPositionTracker(globalModel.getSelectedShip(), worldMousePos));
			//					}
			//				}
			//
			//				// Handling shoot
			//				if (Mouse.getEventButton() == 1 && !Mouse.getEventButtonState() && globalModel.getSelectedShip() != null && World.combat) {
			//					globalModel.getSelectedShip().getIAList().add(new WorldPositionFirer(globalModel.getSelectedShip(), worldMousePos));
			//				}
			//
